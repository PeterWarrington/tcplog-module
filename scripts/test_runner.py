...
from to_qvis import write_qvis as _write_qvis
...
def write_qvis(mininet_results, output_path):
    if sys.argv.count("--qvis") == 1:
        _write_qvis(mininet_results, output_path)
...
class TestTimeoutRetransmission(unittest.TestCase):
    ...
    def setUpClass(self):
        def wait_func(duration):
            ...
            os.system(f"tc qdisc replace dev s1-eth1 root netem loss 100% delay 40ms")
            time.sleep(duration*(1/3))
            os.system(f"tc qdisc replace dev s1-eth1 root netem loss 0% delay 40ms")
            time.sleep(duration*(1/3))
        setup_mininet(self,
                      {"delay": 40, "host_count": 2, "loss": 0},
                      wait_func=wait_func,
                      capture_args={"source_ip":"10.0.0.1", "destination_ip": "10.0.0.2", "max_connections": 1})
...
class TestAggressiveReorder(unittest.TestCase):
    ...
    def setUpClass(self):
        def wait_func(duration):
            time.sleep(duration*(1/3))
            os.system(f"tc qdisc replace dev s1-eth1 root netem delay 50ms reorder 100% gap 5")
            time.sleep(duration*(1/3))
            os.system(f"tc qdisc replace dev s1-eth1 root netem delay 50ms reorder 0% gap 0")
            time.sleep(duration*(1/3))
        setup_mininet(self,
...
class TestSuddenLoss(unittest.TestCase):
    ...
    def setUpClass(self):
        def wait_func(duration):
            ...
            os.system(f"tc qdisc replace dev s1-eth1 root netem loss random 0.5%")
            time.sleep(duration*(1/5))
            os.system(f"tc qdisc replace dev s1-eth1 root netem loss random 0%")
            time.sleep(duration*(1/5))
            os.system(f"tc qdisc replace dev s1-eth1 root netem loss random 0.5%")
            time.sleep(duration*(1/5))
            os.system(f"tc qdisc replace dev s1-eth1 root netem loss random 0%")
    ...
    def test_is_prr(self):
        prr_enter_events = [(i, e) for (i, e) in enumerate(self.mininet_events) if (
            ...
            i > 1
            and
            self.mininet_events[i-1]["data"]["state_variables"]["prr_out"] > 0
        )]

        has_cwnd_decrease = False
        for (prr_enter_i, prr_enter_e) in prr_enter_events:
            ...
            local_all_cwnd_decreases = True
            for i, e in list(enumerate(prr_decrease_events))[1:-1]:
                if e["data"]["state_variables"]["cwnd"] > prr_decrease_events[i-1]["data"]["state_variables"]["cwnd"]:
                    local_all_cwnd_decreases = False
            has_cwnd_decrease = has_cwnd_decrease or local_all_cwnd_decreases
        self.assertTrue(has_cwnd_decrease, "Verify that cwnd decreases during prr.")
...
if __name__ == '__main__':
    ...
    unittest.main(argv=[arg for arg in sys.argv if arg != "--qvis"])
